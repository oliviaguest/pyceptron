...
\begin{tabular}{r S[tabformat=1.1] r}% syntax for siunitx v2; for v1 use "tabformat"
\visible<5->{$1\times0.5~=$&$0.5$&\\}
\visible<6->{$1\times0.0~=$&$0.0$&\\}
\visible<7->{$0\times0.8~=$ & $0.0$& \visible<8->{+\\ \hline}}
\visible<9->{& $0.5$ &}
\end{tabular}
...
@1def Train(self):@      
@2      x = self.input_units@ 
@2      w = self.weights@
@2      h = self.learning_rate@
@2      
@3      for n in range(100):@
@3	    for p in range(len(self.patterns)):@     
@4		  for i in range(len(self.patterns[p])):@
@4		    x[i] = self.patterns[p][i]@
@5		  y = 0@
@5		  for i in range(1+len(self.patterns[p])):@
@5		    y += x[i] * w[i]@
@6		  self.output_units[0] = f(y)@
@6		  error = self.targets[p][0] - self.output_units[0]@
@1		  for i in range(1+len(self.patterns[p])):@
@1		    w[i] += h * error * x[i]@
@1		  self.Draw()@
\end{lstlisting}
